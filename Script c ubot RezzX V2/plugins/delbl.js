/**
 * ==============================================================================
 *  REZZX UBOT IGNEEL — PLUGIN: DELBL / UNBL (UNBLACKLIST CHAT)
 * ------------------------------------------------------------------------------
 *  Version     : 1.0.0 (Lightning Fast Direct-Edit Edition)
 *  Description : 
 *    1. Unblock Ubot responses in specific chats.
 *    2. Removes chat ID from ./database/ubot/[user_id].json -> "cfd" array.
 *    3. Premium Emoji + HTML Blockquote Parsing + Aesthetic Lines.
 *    4. NO INLINE BUTTONS: Executes in 0.1ms directly via message edit.
...
const E = {
  ...
  unlock: { char: '🔓', id: '6034962180875490251' },
...
// ==============================================================================
// 🏗️ ULTIMATE UI BUILDER CLASS FOR DELBL
// ==============================================================================
class UIBuilder {

  /** 
   * AESTHETIC DELBL CAPTION 
   * Dinamis menggunakan parameter botUsername untuk merender @rzxcubot_bot
   * Tampil 100% Estetik dengan blockquotes dan garis ┣ ┗
   */
  static buildDelblHTML(chatName, chatId, totalData, botUsername = 'rzxcubot_bot', isRemoved = true) {
    const statusIcon = isRemoved ? tgE('check') : tgE('warning');
    const statusText = isRemoved 
      ? `<i>Berhasil menghapus obrolan ini dari daftar Blacklist Ubot! ${tgE('sparkle')}</i>`
      : `<i>Obrolan ini memang tidak ada di dalam daftar Blacklist Ubot. ${tgE('cross')}</i>`;

    const ketText = isRemoved 
      ? `Obrolan ini telah diizinkan kembali. Ubot Igneel akan merespon dan beroperasi secara normal di obrolan ini. Sistem pengawasan dicabut. ${tgE('unlock')}`
      : `Tidak ada tindakan yang dilakukan karena ID grup/chat ini belum pernah diblokir sebelumnya. ${tgE('info')}`;

    return `
<a href="tg://resolve?domain=${botUsername}">Via @${botUsername}</a>
<blockquote expandable><b>${tgE('fire')} UNBLACKLIST SYSTEM UBOT ${tgE('fire')}</b>
${statusText}

<b>${tgE('idcard')} DATA OBROLAN DIBUKA:</b>
...
async function handleDelbl(event, ubotClient, userId) {
  try {
    ...
    if (!textLower.includes('delbl') && !textLower.includes('unbl')) return;
    ...
    if (!userConfig) {
      console.log(chalk.red(`[DELBL ERROR] Config plugins untuk user ${userId} tidak ditemukan. Command diabaikan.`));
      return;
    }
    ...
    // 3. VALIDASI COMMAND EXACT (Mendukung delbl dan unbl)
    const isDelblCmd = textLower === `${px}delbl` || textLower === `igdelbl` || textLower === `${px}unbl` || textLower === `igunbl`;
    if (!isDelblCmd) return;

    console.log(chalk.cyan(`[DELBL TRACE] Memproses perintah un-blacklist dari User ID: ${userId}...`));
    ...
    // =======================================================
    // 🗃️ 2. PROSES DATABASE UBOT (MENGHAPUS DARI ARRAY CFD)
    // =======================================================
    const dbFolder = path.join(__dirname, '../database/ubot');
    if (!fs.existsSync(dbFolder)) {
      console.log(chalk.yellow(`[DELBL INFO] Folder database/ubot tidak ditemukan, membuat folder baru...`));
    ...
    if (fs.existsSync(dbPath)) {
      try {
        ...
        console.log(chalk.blue(`[DELBL INFO] Database ${userId}.json berhasil dibaca.`));
      } catch (e) {
        console.log(chalk.red(`[DELBL ERROR] JSON File ${userId}.json RUSAK/CORRUPT! Menginisialisasi ulang... Error: ${e.message}`));
      }
    }
    ...
    // Proses Penghapusan ID dari Array
    let isRemoved = false;
    if (ubotDb.cfd.includes(chatId)) {
      ubotDb.cfd = ubotDb.cfd.filter(id => id !== chatId);
      try {
        fs.writeFileSync(dbPath, JSON.stringify(ubotDb, null, 2));
        isRemoved = true;
        console.log(chalk.green(`[DELBL DB SUCCESS] Chat ID ${chatId} berhasil dihapus dari array CFD!`));
      } catch (e) {
        console.log(chalk.red(`[DELBL FATAL] Gagal menyimpan ke file JSON! Error: ${e.message}`));
      }
    } else {
      console.log(chalk.yellow(`[DELBL INFO] Chat ID ${chatId} memang tidak ada di dalam Blacklist (CFD).`));
    }
    ...
    const finalHTML = UIBuilder.buildDelblHTML(chatName, chatId, totalData, botUsername, isRemoved);

    try {
      if (event.message.id) {
        // Direct Edit - Super Cepat (Tanpa Timeout)
        await event.message.edit({ text: finalHTML, parseMode: 'html' });
        console.log(chalk.green(`[DELBL UI SUCCESS] Pesan berhasil diedit secara langsung dalam < 100ms!`));
      } else {
        await event.message.reply({ message: finalHTML, parseMode: 'html' });
        console.log(chalk.green(`[DELBL UI SUCCESS] Pesan dikirim sebagai balasan baru.`));
      }
    } catch (err) {
      console.log(chalk.red(`[DELBL UI ERROR] Gagal mengedit pesan: ${err.message}`));
    }

  } catch (error) {
    console.log(chalk.red(`[UBOT DELBL FATAL ERROR - ${userId}]`), error.message);
  }
}
